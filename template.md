...
- 前序表达式（波兰表达式）求值：
  ...
  - 递归定义波兰表达式，再用递归求解：波兰表达式是一个数，或者“操作符 波兰表达式 波兰表达式”，[参考代码](http://cs101.openjudge.cn/2024sp_routine/solution/44065119/)；这是一个**化整为零**的过程
...
- ==Fix an ordering for variables==, and select values for variables in this order. ==This ensures that no duplicates occur.==
...
- 和闫氏 dp 分析法一样，从集合的角度分析，每个（不一定符合要求的）可能方案都是一个点，这些点组成一个集合，称为全集 $U$
- 题目一般对方案作出两方面限制，一方面有可行域 $V$（例如变量的取值范围），另一方面是约束条件；二者其实都是约束条件，但这里先稍作区分以便表述
- 枚举和搜索，就是从全集 $U$​ 中，寻找既在可行域内，又满足约束方程的点，或者更进一步的在这些点中再挑出最大、最小或其他点
- 在枚举和搜索时，我们往往最先想到的思路是，在可行域 $V$ 内，穷尽所有可能方案，逐一检查是否符合约束条件
- 但如果 $V$ 很大的话，这样做的时间复杂度是很高的
- 我们注意到，可行域和约束方程，本质都是对可能方案的一种约束条件，因此我们枚举时可以把一部分（方便的）约束方程，归到可行域中去，即取出 $V$ 和某一部分约束方程的交集 $V'$ 作为新的可行域，此时需要枚举的方案数就降低了
- 例如，八皇后中，我们“自动地”把 queen 不在同一行归到了可行域中去，略去了很多不符合要求的情况；又如，[完美立方](http://cs101.openjudge.cn/2024sp_routine/02810/)中可以不枚举`a`：[题解解法](http://cs101.openjudge.cn/2024sp_routine/solution/44020161/)
...
==挪到递归的笔记里去==在递归问题（包括递归形式的 backtracking search）中，我们往往需要在函数的参数中记录一些信息（例如搜索深度），这里一般来说可以按两种方式来记录：
