...
#### 41. 三元表达式易错点
三元表达式`expr1 if condition else expr2` (其实就是 C++ 的`condition ? expr1 : expr2`) 中常因为`expr1`前面还有表达式, 不添加**括号**就会导致前面的表达式也被判定为`expr1`, 例如:
```Python
print(f'Case #{case}: ' + ('x' if flag else 'y'))
```
如果不加括号, `flag`为假时输出的就只有`'y'`了.
