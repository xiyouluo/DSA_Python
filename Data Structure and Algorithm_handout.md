...
动态规划好歹是无冗余地穷举所有解, 然后找一个最值, 但贪心算法甚至不用穷举所有解就可以找到答案, 所以其效率比动态规划还高. 当然, 并不是所有问题都存在贪心选择性质让我们"投机取巧", 所以全量穷举虽然朴实无华且枯燥, 但真的是任何情况下都可以用的. 
...
`N` 叉树的遍历又可以进一步扩展为图的遍历, 因为图就是好几 `N` 叉棵树 (搜索树) 的结合体. 但需要注意图中是可能出现环的, 需要在适当的时候用 `visited` 做标记, 这里代码从略, 什么时候标记 (什么时候一个节点无需再访问/该节点无需再扩展) 在[[#3. 搜索、图的遍历]]中有详细的讲解.

**所谓框架, 就是套路. 不管增删查改, 这些代码都是永远无法脱离的结构**, 你可以把这个结构作为大纲, 根据具体问题在框架上添加代码就行了.
### 2.3 二叉树做题总结
由上可以看出, 对于整个数据结构与算法而言, 如果非要说一个最重要的框架, 那就是二叉树的框架.
...
此处先总结线性数据结构及相关的算法, 非线性的部分在树和图两章总结.
## 1. 栈和队列
### 1.1 栈
C++ 数组实现: (这样写会快一些)
```cpp
...
[reference](https://labuladong.online/algo/essential-technique/binary-tree-summary/#深入理解前中后序)
...
## \*10. Segment Tree
对一个数组进行查改. 以查询区间和为例:
1. 如果要改很多次, 只查一次 (或少量次), 就需要降低改的时间复杂度. 这是非常 trivial 的情形, 就只需要存储该数组, 显然改是 $O(1)$ 的, 查是 $O(n)$ 的 (计算总和).
2. 如果只需要改少量次 (或者不改), 但要查很多次, 就需要降低查的时间复杂度. 这也是很简单的情形, 只需要维护前缀和, 此时改是 $O(n)$ 的 (重新维护前缀和), 查是 $O(1)$ 的. 

如果查和改都是多次, 就需要兼顾二者, 让 $max\{t_{update}, t_{query}\}$ 尽量小, 也就是让目前的一大一小尽量往中间靠 (类似[[#重整约束]]中提到的双向搜索的优化逻辑). 最后我们有结论, 二者往中间靠的结果是 $O(\log(n))$, 实现为线段树.
